...
from experiments.mor_pinn.wave_eq_rom import run_model


lower_bounds = [4, 40, 1e-4]

upper_bounds = [8, 60, 1e-3]
...
config.n_boundary = 1000
config.n_interior = 10000
config.n_rom = 10000


if args.opt:
    config.optimal_weighting = True
    config.model_dir = "/import/sgs.local/scratch/leiterrl/h_param_wave_eq_pinn_mor_opt"
elif args.lra:
    config.lra = True
    config.model_dir = "/import/sgs.local/scratch/leiterrl/h_param_wave_eq_pinn_mor_lra"
else:
    config.model_dir = (
        "/import/sgs.local/scratch/leiterrl/h_param_wave_eq_pinn_mor_equal"
    )
