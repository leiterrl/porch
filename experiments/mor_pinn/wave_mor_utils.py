...
class OnedScipyWaveEquationModel(WaveEquationModel):
    def __init__(
        ...
        operator = LincombOperator(
            ...
            [ExpressionParameterFunctional("-wave_speed**2", {"wave_speed": 0.0})],
        )
...
class TravellingWaveEquationModel(OnedScipyWaveEquationModel):
    def __init__(
        ...
        initial_data = InterpolationOperator(
            grid,
            ExpressionFunction(
                "(x >= -q0_supp/2) * (x <= q0_supp/2) * 1/2 * (1 + cos(2*pi/q0_supp * x))",
        ...
        if initial_velocity_type == "zero":
            ...
            initial_velocity = InterpolationOperator(
                grid,
                ExpressionFunction(
                    "(x >= -q0_supp/2) * (x <= q0_supp/2) * pi/q0_supp * sin(2*pi/q0_supp * x)",
                    parameter_type={"q0_supp": ()},
                ),
            )
            initial_velocity = LincombOperator(
                [VectorOperator(initial_velocity.as_vector({"q0_supp": q0_supp}))],
                [ExpressionParameterFunctional("-wave_speed", {"wave_speed": 0.0})],
            )
        else:
        ...
        super().__init__(
            ...
            parameter_space=CubicParameterSpace(
                ParameterType({"wave_speed": 0.0}), ranges={"wave_speed": (-2, 2)}
            ),
        )
...
class GlasEtAlWaveEquationModel(OnedScipyWaveEquationModel):
    def __init__(
        ...
        first_node_dirichlet_mass = space.make_array(
            np.hstack([1, np.zeros(space.dim - 1)])
        )
        ...
        rhs = LincombOperator(
            ...
            [
                ExpressionParameterFunctional(
                    "150/(cosh(5*_t)**2) * tanh(5*_t) * (1/(cosh(5*_t)**2) - tanh(5*_t)**2)",
                    {"_t": 0.0},
                ),
                ExpressionParameterFunctional(
                    "wave_speed**2 * tanh(5*_t)**3", {"_t": 0.0, "wave_speed": 0.0}
                ),
            ],
        )
        ...
        super().__init__(
            ...
            visualizer=visualizer,
            parameter_space=CubicParameterSpace(
                ParameterType({"wave_speed": 0.0}), ranges={"wave_speed": (0.3, 2)}
            ),
        )
...
class WaveEquationTimeStepper(TimeStepper):
    ...
    def __init__(self, nt, theta=0.25):
    ...
    def solve(
        ...
        if mass is None:
            mass = NumpyMatrixOperator(
                eye(so_space.dim), source_id=so_space.id, range_id=so_space.id
            )
        ...
        for _ in np.arange(2, nt):
            ...
            if b:
                if b_time_dep:
                    g = b.as_vector(mu)
                    dtsq_gh = (
                        theta * g + (1 - 2 * theta) * g_old + theta * g_old_old
                    ) * dt ** 2
                    g_old_old = g_old
                    g_old = g
...
def eval_rom(
    ...
    updated_attributes = {
        "time_stepper": rom.time_stepper.with_(nt=n_time_steps),
        "initial_time": initial_time,
        "T": final_time,
    }
...
def generate_fom(scenario):
    ...
    if scenario in ("zero", "travelling"):
        ...
        fom = TravellingWaveEquationModel(
            T,
            n_spatial,
            initial_velocity_type=scenario,
            time_stepper=time_stepper,
            q0_supp=q0_supp,
        )
        n_wave_speed = 4
        ...
        max_extensions = 33
        rtol = (
            None  # enforce 33 iterations in greedy to repeat experiments of the paper
        )
    else:
